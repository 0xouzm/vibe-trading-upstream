...
- **2026-09-25** 🛠️ **リサーチセッションとチャネル診断を改善**：バックテスト CSV に文字列の列があっても数値指標が失われなくなり（[#1579](https://github.com/HKUDS/Vibe-Trading/pull/1579)）、会話圧縮で保持するメッセージの容量計算に推論内容を含めるようになりました（[#1582](https://github.com/HKUDS/Vibe-Trading/pull/1582)）。チャネルの読み込み失敗時には実際の例外を記録し（[#1580](https://github.com/HKUDS/Vibe-Trading/pull/1580)）、Telegram ガイドでは CLI と Web の操作が同じ API ランタイムを共有することを明記しました（[#1583](https://github.com/HKUDS/Vibe-Trading/pull/1583)）。Robinhood のポートフォリオ取得は不正な形式の購買力オブジェクトを拒否し、省略値や null は不明のまま保持します（[#1526](https://github.com/HKUDS/Vibe-Trading/pull/1526)）。[@Shizoqua](https://github.com/Shizoqua) と [@lorenzozanee](https://github.com/lorenzozanee) に感謝！
